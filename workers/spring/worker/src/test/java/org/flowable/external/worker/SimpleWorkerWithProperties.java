 ...
 */
@Component
public class SimpleWorkerWithProperties {

    protected final List<AcquiredExternalWorkerJob> receivedJobs = new CopyOnWriteArrayList<>();

    @FlowableWorker(topic = "${topic}", lockDuration = "${lockDuration}", numberOfTasks = "${numberOfTasks}", numberOfRetries = "${numberOfRetries}", pollingInterval = "${pollingInterval}")
